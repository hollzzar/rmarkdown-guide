...
Just as in a code chunk, I have to specify that I'm working with R code by including a lower-case r in the backticks with the variable. 

So if I include `r knitr::inline_expr('species_count')` in the text, I can report that there are `r species_count` species without writing out the number itself.

I can reference the variable with the exact species names---`r knitr::inline_expr('species_count')`---in the same way: `r species`.